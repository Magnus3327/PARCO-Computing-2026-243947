...
def plot_efficiency_annotate_right(sequential_file, parallel_file, output_folder):
    ...
    for idx, (matrix, vals_by_thread) in enumerate(best_by_matrix.items()):
        ...
        for t in threads:
            if t == 1:
                ...
                speedup = vals_by_thread[t][0]
                efficiency = speedup / t
                efficiencies.append(efficiency)
                scheds.append(vals_by_thread[t][1])
                chunks.append(vals_by_thread[t][2])
        color = color_palette[idx % len(color_palette)]
        plt.plot(threads, efficiencies, marker='o', color=color, label=matrix, linewidth=2)
        ...
        plt.scatter([max_thread], [max_eff], color=color, marker='v', s=160, edgecolors='k', zorder=10)
        annotation = f"Eff:{max_eff:.2f}, scheduling: {max_sched}, chunkSize: {max_chunk}"
        plt.annotate(
            annotation,
            xy=(max_thread, max_eff),
            xytext=(max_thread, max_eff + 0.03),
            fontsize=9,
            color=color,
            ha='left',
            va='center'
    ...
    plt.ylabel('Parallel Efficiency (Speedup / Threads)')
    plt.title('Parallel Efficiency vs Threads per Matrix\nUsing Best Configuration per Thread Count')
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python plot_efficiency.py <sequential.json> <parallel.json> <output_folder>")
        sys.exit(1)
    sequential_file = sys.argv[1]
    parallel_file = sys.argv[2]
    output_folder = sys.argv[3]
    plot_efficiency_annotate_right(sequential_file, parallel_file, output_folder)
